...
Java_com_nzy_mediacodecpush_ScreenLive_connect(JNIEnv *env, jobject thiz, jstring url_) {
    ...
    do {
        ...
        LOGE("connect success ret %d code = %d" ,ret , code);
    } while (0);
    if (!ret && live) {
        free(live);
        live = nullptr;
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_nzy_mediacodecpush_ScreenLive_sendData(JNIEnv *env, jobject thiz, jbyteArray data_,
