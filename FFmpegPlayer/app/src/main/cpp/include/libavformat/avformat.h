 ...
 * @li an @ref AVFormatContext.pb "I/O pAvFormatContext". It is either opened by lavf or
 ...
 * Further options may be provided by the @ref AVFormatContext.pb "I/O pAvFormatContext",
 ...
 * options on a preallocated pAvFormatContext. Instead, the options should be passed to
 ...
 * avformat_alloc_context() to create a muxing pAvFormatContext. The caller then sets up
 * the muxer by filling the various fields in this pAvFormatContext:
 *
 * - The @ref AVFormatContext.oformat "oformat" field must be set to select the
 *   muxer that will be used.
 * - Unless the format is of the AVFMT_NOFILE type, the @ref AVFormatContext.pb
 *   "pb" field must be set to an opened IO pAvFormatContext, either returned from
 ...
 *   remuxing: there is no guarantee that the codec pAvFormatContext values remain valid
 ...
 *   support.
 *
 * When the muxing pAvFormatContext is fully set up, the caller must call
 * avformat_write_header() to initialize the muxer internals and write the file
 * header. Whether anything actually is written to the IO pAvFormatContext at this step
 ...
 * a single muxing pAvFormatContext, they should not be mixed). Do note that the timing
 ...
 * pAvFormatContext (if any) and finally free the muxing pAvFormatContext with
 ...
 *
 * @param s    associated IO pAvFormatContext
 * @param pkt packet
...
typedef struct AVOutputFormat {
    ...
    const AVClass *priv_class; ///< AVClass for the private pAvFormatContext
...
typedef struct AVFormatContext {
    ...
    /**
     * I/O pAvFormatContext.
     *
     * - demuxing: either set by the user before avformat_open_input() (then
     *             the user must close it manually) or set by avformat_open_input().
     * - muxing: set by the user before avformat_write_header(). The caller must
     *           take care of closing / freeing the IO pAvFormatContext.
     *
...
#define AVFMT_AVOID_NEG_TS_MAKE_ZERO         2 ///< Shift timestamps so that they start at 0
    ...
    /**
     * Flush the I/O pAvFormatContext after each packet.
     * - encoding: Set by user
     * - decoding: unused
     */
    int flush_packets;
     ...
     * This is set by avformat when the underlaying IO pAvFormatContext read pointer
     ...
     * other times as well), it will call this callback to obtain an IO pAvFormatContext.
     *
     * @param s the format pAvFormatContext
     * @param pb on success, the newly opened IO pAvFormatContext should be returned here
 ...
 * avformat_free_context() can be used to free the pAvFormatContext and everything
 ...
 * @param s pAvFormatContext to free
 */
 ...
 * avformat_free_context() can be used to free the pAvFormatContext and
 * everything allocated by the framework within it.
 *
 * @param *ctx is set to the created format pAvFormatContext, or to NULL in
 * case of failure
 * @param oformat format to use for allocating the pAvFormatContext, if NULL
 * format_name and filename are used instead
 * @param format_name the name of output format to use for allocating the
 * pAvFormatContext, if NULL filename is used instead
 * @param filename the name of the filename to use for allocating the
 * pAvFormatContext, may be NULL
 ...
 * @param logctx the log pAvFormatContext
 ...
 *
 * @note If you want to use custom IO, preallocate the format pAvFormatContext and set its pb field.
 */
 ...
 * codec and time base.
 *
 * @param ic        the pAvFormatContext to analyze
 * @param index     index of the stream to dump information about
 * @param url       the URL to print, such as source or destination file
 * @param is_output Select whether the specified pAvFormatContext is an input(0) or output(1)
 */
 ...
 *           array is composed by only one pAvFormatContext, such pAvFormatContext can contain
 ...
 *
 * @param format the format pAvFormatContext which the stream is part of
 ...
 *
 * @param ctx the format pAvFormatContext which the stream is part of
 * @param stream the stream which the frame is part of
