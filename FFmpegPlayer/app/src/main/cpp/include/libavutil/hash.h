 ...
 * hash_name = ...;
 *
 * // Allocate a hash pAvFormatContext
 * ret = av_hash_alloc(&ctx, hash_name);
 * if (ret < 0)
 *     return ret;
 *
 * // Initialize the hash pAvFormatContext
 * av_hash_init(ctx);
 *
 * // Update the hash pAvFormatContext with data
 * while (data_left) {
 ...
 *
 * // Finalize the hash pAvFormatContext.
...
/**
 * Allocate a hash pAvFormatContext for the algorithm specified by name.
 *
 * @return  >= 0 for success, a negative error code for failure
 *
 * @note The pAvFormatContext is not initialized after a call to this function; you must
...
/**
 * Get the name of the algorithm corresponding to the given hash pAvFormatContext.
 */
 ...
 * #AV_HASH_MAX_SIZE.
 *
 * @param[in]     ctx Hash pAvFormatContext
 * @return            Size of the hash value in bytes
 */
int av_hash_get_size(const struct AVHashContext *ctx);

/**
 * Initialize or reset a hash pAvFormatContext.
 *
 * @param[in,out] ctx Hash pAvFormatContext
 */
void av_hash_init(struct AVHashContext *ctx);

/**
 * Update a hash pAvFormatContext with additional data.
 *
 * @param[in,out] ctx Hash pAvFormatContext
 * @param[in]     src Data to be added to the hash pAvFormatContext
...
/**
 * Finalize a hash pAvFormatContext and compute the actual hash value.
 *
 * The minimum size of `dst` buffer is given by av_hash_get_size() or
 * #AV_HASH_MAX_SIZE. The use of the latter macro is discouraged.
 *
 * It is not safe to update or finalize a hash pAvFormatContext again, if it has already
 * been finalized.
 *
 * @param[in,out] ctx Hash pAvFormatContext
...
/**
 * Finalize a hash pAvFormatContext and store the actual hash value in a buffer.
 *
 * It is not safe to update or finalize a hash pAvFormatContext again, if it has already
 * been finalized.
 *
...
/**
 * Finalize a hash pAvFormatContext and store the hexadecimal representation of the
...
/**
 * Finalize a hash pAvFormatContext and store the Base64 representation of the
 * actual hash value as a string.
 *
 * It is not safe to update or finalize a hash pAvFormatContext again, if it has already
 * been finalized.
...
/**
 * Free hash pAvFormatContext and set hash pAvFormatContext pointer to `NULL`.
 *
 * @param[in,out] ctx  Pointer to hash pAvFormatContext
 */
