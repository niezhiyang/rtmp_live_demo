...
Java_com_nzy_mediacodec_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
...
Java_com_nzy_mediacodec_LivePush_connect(JNIEnv *env, jobject thiz, jstring url_) {
    ...
    do {
        ...
        int code  = RTMP_IsConnected(live->rtmp);

        LOGE("connect success ret %d" ,ret);
    } while (0);
    if (!ret && live) {
        free(live);
        live = nullptr;
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_nzy_mediacodec_LivePush_sendData(JNIEnv *env, jobject thiz, jbyteArray data_, jint len,
                                          jlong tms, jint type) {
    int ret;
    ...
    switch (type) {
        case 0: //video
            ...
            LOGI("send Video  lenght :%d %d",len,ret);
            break;
        default: //audio
            ret = sendAudio(data, len, type, tms);
            LOGI("send Audio  lenght :%d %d",len,ret);
            break;
    }
